...
// user_id). Functions here convert between that key and the two shapes below.

export type InboundRef = { chatType: string; chatId: string; threadId?: number }

export type Target = { chat_id: string; thread_id?: number } // no thread_id = DM or plain group
...
export function keyToTarget(key: string): Target {
  ...
  if (!m) {
    throw new Error(`bad binding key "${key}"`)
  }
...
export function targetFor(keys: string[], chatId?: string, threadId?: string): Target {
  if (chatId) {
    if (threadId) {
      return { chat_id: chatId, thread_id: Number(threadId) }
    }
    ...
    if (topics.length > 1) {
      throw new Error(`several topics bound for chat ${chatId} — pass thread_id`)
    }
    return { chat_id: chatId }
  }
  if (keys.length === 1) {
    return keyToTarget(keys[0])
  }
  throw new Error(
    keys.length === 0
