...
## Stage 3 — пермишены (нужна non-bypass сессия)

**RP5. Запрос пермишена переживает рестарт.** `[СПЕЦ]`
- Предусловие: сессия БЕЗ `bypassPermissions`.
- Шаги: агент вызывает тул, требующий разрешения → кнопки allow/deny админам → `pkill hub` → поднять
  → нажать «Разрешить».
- Ожидаемо: `hub-state.json.permissions[request_id]` есть; после boot `resolvePermission` находит
  живой conn биндинга (не мёртвый сокет) и досылает allow → агент продолжает.
- Fail-режим: кнопка «🤷 Сессия ушла» хотя сессия жива (не нашёл conn); агент завис.
- Смотреть: `hub-state.json`, чат, пейн (тул выполнился).
